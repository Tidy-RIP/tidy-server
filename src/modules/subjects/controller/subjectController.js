...
export const getUserSubjectsCtrl = async (req, res) => {
  ...
  try {
    logger.info(
      `Solicitud para obtener todas las materias para el usuario con id: ${userId}`
    );
...
export const createSubjectCtrl = async (req, res) => {
  ...
  try {
    logger.info(
      `Solicitud para crear una nueva materia para el usuario con id: ${userId}`
    );
    ...
    logger.error(
      `Error en el controlador al crear la materia para el usuario con id: ${userId}. Su error es: ${error.stack}`
    );

    res.status(error.statusCode || 500).json({
      message:
        "Error en el servidor al crear la materia. Por favor, intentalo de nuevo.",
    });
  }
};
...
export const updateSubjectCtrl = async (req, res) => {
  ...
  try {
    logger.info(
      `Solicitud para editar la materia con id: ${id} para el usuario con id: ${userId}`
    );
...
export const deleteSubjectCtrl = async (req, res) => {
  ...
  try {
    logger.info(
      `Solicitud para eliminar la materia con id: ${id} para el usuario con id: ${userId}`
    );
    ...
    logger.error(
      `Error en el controlador al eliminar la materia con id: ${id}. Su error es: ${error.stack}`
    );
