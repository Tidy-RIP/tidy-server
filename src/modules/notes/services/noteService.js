...
export const findNotesById = async noteId => {
  try {
    ...
    throw createError('Error en el servidor al obtener la nota', error.statusCode || 500);
  }
};
...
export const findNotesBySubjectId = async subjectId => {
  try {
    ...
    if (!notes || notes.length === 0) {
      logger.error(`No se encontraron notas en la materia con id: ${subjectId}`);
      throw createError('No se encontraron notas en la materia', 404);
    }

    return notes;
  } catch (error) {
    logger.error(`Error en el servidor al buscar las notas de la materia con id: ${subjectId}. Su error es: ${error.stack}`);

    throw createError('Error en el servidor al buscar las notas de la materia', error.statusCode || 500);
  }
};
...
export const createNote = async noteData => {
  ...
  try {
    ...
    throw createError('Ocurrio un error al crear una nueva nota', error.statusCode || 500);
  }
};
...
export const updateNote = async (noteId, noteData) => {
  try {
    ...
    logger.error(`Ocurrio un error en el servidor al actualizar la nota con id: ${noteId}`);

    throw createError('Ocurrio un error al actualizar la nota', error.statusCode || 500);
  }
};
...
export const deleteNote = async noteId => {
  try {
    ...
    logger.error(`Ocurrio un error en el servidor al eliminar la nota con id: ${noteId}`);

    throw createError('Ocurrio un error al eliminar la nota', error.statusCode || 500);
  }
};
